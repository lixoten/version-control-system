...
class Vsc (args: Array<String>) {
    ...
    fun run() {
        ...
        (when (command) {
            ...
            "checkout" -> println(commandCheckout(param))
    ...
    fun commandCheckout(commitId: String):String {
        if (commitId == "") {
            ...
            if (!commitIdExists(commitId)){
                ...
                return restoreFile(commitId)
            }
        }
    }

    fun commitIdExists(commitId: String): Boolean {
        val commitDirIds = commitDir.listFiles()!!.map{it.name}
        if (commitDirIds.isNotEmpty()){
            return if (commitDirIds.contains(commitId)) true else false
        }
        return false
    }

    fun restoreFile(commitId: String): String {
        val myFiles = File(COMMIT_DIRECTORY + commitId).listFiles()

        for (file in myFiles) {
            val fileIn = File(COMMIT_DIRECTORY + "$commitId\\${file.name}")
            val fileOut = File(file.name)
            fileIn.copyTo(fileOut, overwrite = true)
        }
...
fun main(args: Array<String>) {
    ...
    //args = arrayOf("log")
    //args = arrayOf("checkout", "assfd") // See below note #1
    //args = arrayOf("checkout", "c62c2346ba405ae2483d2f729e73240d")
